...
4. **주의할 점 (필수)**: 배포 시 **Python 3.12**를 반드시 선택해야 합니다. Streamlit Cloud 기본값(3.14)은 Pillow 등 의존성 빌드 실패를 유발합니다.
   - **배포 시**: Create app → Advanced settings → **Python version: 3.12** 선택 후 저장
   - **이미 배포한 앱이 실패하는 경우**: 앱 삭제 후 다시 배포하면서 Advanced settings에서 Python 3.12 선택
   - 프로젝트 루트에 `runtime.txt`(python-3.12)를 포함해 두었습니다. 
