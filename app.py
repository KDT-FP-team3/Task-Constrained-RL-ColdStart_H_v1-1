...
fig_main.add_trace(go.Scatter(x=[0], y=[0], mode='lines+markers', name='<b>KOSPI 지수</b>', line=dict(color='green', width=2, dash='dot'), marker=dict(symbol='diamond-open', size=6)))
...
if st.button("Run Evaluation"):
    for run in range(auto_runs):
        ...
        for i in range(20, 20 + episodes):
            ...
            bench_col = env.benchmark if env.benchmark in env.data.columns else None
            if bench_col is not None:
                sc, sn = float(env.data[bench_col].iloc[i]), float(env.data[bench_col].iloc[i+1])
            ...
            m_b.metric(label="KOSPI 지수", value=f"{h_b[-1]:.2f}%", delta=f"{r_b:.2f}%")
...
if len(st.session_state.trial_history) > 0:
    ...
    with col_box:
        ...
        fig_box.add_annotation(x=1.625, xref="x", y=avg_kospi, text=f"<b>KOSPI 지수<br>{avg_kospi:.2f}%</b>", showarrow=False, yshift=18, xanchor='center', align='center', font=dict(color="green", size=13, family="Arial Black"), bgcolor="rgba(0,0,0,0)")
    ...
    with col_tbl_h:
        ...
        st.dataframe(df_h.set_index("Trial").style.map(style_df).format({"Vanilla Final (%)": "{:.2f}", "STATIC Final (%)": "{:.2f}", "KOSPI Final (%)": "{:.2f}", "Seed": "{:.0f}"}), height=320, use_container_width=True)